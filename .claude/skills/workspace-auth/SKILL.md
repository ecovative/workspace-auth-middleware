...
- **Admin SDK for Business Standard:** See [references/admin-sdk.md](references/admin-sdk.md)
...
- `fetch_groups=True` requires credentials (see below)
- `fetch_groups=False` requires no credentials (token-only auth)
- `delegated_admin` switches from Cloud Identity to Admin SDK Directory API
- `target_groups` enables efficient transitive group resolution via BFS
